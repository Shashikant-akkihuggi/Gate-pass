...
const Approvals = () => {
    ...
    const [actionModal, setActionModal] = useState({ isOpen: false, type: '', pass: null });
    ...
    const fetchData = async () => {
        try {
            ...
            // Safely extract approvals array from response
    ...
    const handleAction = (type, pass) => {
        setActionModal({ isOpen: true, type, pass });
        setRemarks('');
    };
    ...
    const handleSubmitAction = async () => {
        ...
        try {
            if (actionModal.type === 'approve') {
                await approvalService.approvePass(actionModal.pass.pass_id, remarks);
                toast.success('Pass approved successfully');
            } else {
                await approvalService.rejectPass(actionModal.pass.pass_id, remarks);
                toast.success('Pass rejected successfully');
            }

            setActionModal({ isOpen: false, type: '', pass: null });
            fetchData();
        } catch (error) {
            toast.error(handleError(error));
        } finally {
    ...
    return (
        <div className="space-y-6">
            ...
            <Modal
                ...
                onClose={() => setActionModal({ isOpen: false, type: '', pass: null })}
                title={`${actionModal.type === 'approve' ? 'Approve' : 'Reject'} Pass`}
            >
                <div className="space-y-4">
                    <p className="text-gray-600">
                        Are you sure you want to {actionModal.type} this pass request?
                    </p>
                    ...
                    <Textarea
                        ...
                        placeholder={`Add ${actionModal.type === 'reject' ? 'reason for rejection (required)' : 'optional remarks'}`}
                    ...
                    <div className="flex justify-end space-x-3">
                        <Button
                            variant="secondary"
                            onClick={() => setActionModal({ isOpen: false, type: '', pass: null })}
                        >
                            Cancel
                        </Button>
                        <Button
